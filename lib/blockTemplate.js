...
var defaultSubsidyMultipleOfSatoshi = 100000000;
...
var BlockTemplate = module.exports = function BlockTemplate(
    ...
    let blockReward = {
        'total': (this.rpcData.miner) * (coin.subsidyMultipleOfSatoshi || defaultSubsidyMultipleOfSatoshi)
    };
    ...
    var governanceReward;
    var governancePayee;
    var governancePayment;
    ...
    if (coin.payFoundersReward === true) {
        if (!this.rpcData.founders || this.rpcData.founders.length <= 0) {
            ...
            blockReward = {
                "total": (this.rpcData.miner + this.rpcData.founders + (this.rpcData.treasury || 0) + this.rpcData.securenodes + this.rpcData.supernodes) * (coin.subsidyMultipleOfSatoshi || defaultSubsidyMultipleOfSatoshi)
            };
        }
    }
    ...
    if(coin.VRSEnabled === true) {
        ...
        if(this.rpcData.height >= coin.VRSBlock){
            if (!this.rpcData.vrsReward || this.rpcData.vrsReward.length <= 0) {
                ...
                blockReward = {
                    "total": (this.rpcData.miner * (coin.subsidyMultipleOfSatoshi || defaultSubsidyMultipleOfSatoshi)) + this.rpcData.vrsReward + this.rpcData.payee_amount
                };
            }
        }
        else{ //VRS Ready but not yet activated by chain
            blockReward = {
                "total": (this.rpcData.miner * (coin.subsidyMultipleOfSatoshi || defaultSubsidyMultipleOfSatoshi)) + this.rpcData.payee_amount
            };
        }
    }

    if (coin.symbol === 'psl' || coin.symbol === 'lsp') {
        // console.log('this.rpcData.masternode (' + Object.keys(this.rpcData.masternodeinfo) + ')');
        if (Object.keys(this.rpcData.masternodeinfo).length !== 0){
            masternodeReward = (this.rpcData.masternodereward) * (coin.subsidyMultipleOfSatoshi || defaultSubsidyMultipleOfSatoshi);
            masternodePayee = this.rpcData.masternodeinfo.payee;
        } else {
            masternodeReward = 0
        }
        if (Object.keys(this.rpcData.governanceinfo).length !== 0){
            governanceReward = (this.rpcData.governancereward) * (coin.subsidyMultipleOfSatoshi || defaultSubsidyMultipleOfSatoshi);
            governancePayee = this.rpcData.governanceinfo.payee;
        } else {
            governanceReward = 0
        }
        blockReward = {
            "total": (this.rpcData.miner * (coin.subsidyMultipleOfSatoshi || defaultSubsidyMultipleOfSatoshi)) + masternodeReward + governanceReward
        };
    } else {
    ...
    // console.log('BlockTemplate - masternodeReward (' + masternodeReward + ')');
    // console.log('BlockTemplate - governanceReward (' + governanceReward + ')');
    // console.log('BlockTemplate - blockReward (' + blockReward.total + ')');
