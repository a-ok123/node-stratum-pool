...
var BlockTemplate = module.exports = function BlockTemplate(
    ...
    if (coin.symbol === 'psl' || coin.symbol === 'lsp') {
        if (Object.keys(this.rpcData.masternode).length !== 0){
            masternodeReward = (this.rpcData.masternode) * (coin.subsidyMultipleOfSatoshi || defaultSubsidyMultipleOfSatoshi);
            masternodePayee = this.rpcData.masternode.payee;
        }
        if (Object.keys(this.rpcData.governance).length !== 0){
            governanceReward = (this.rpcData.governance) * (coin.subsidyMultipleOfSatoshi || defaultSubsidyMultipleOfSatoshi);
            governancePayee = this.rpcData.governance.payee;
        }
        blockReward = {
    ...
    this.serializeHeader = function(nTime, nonce){
        ...
        //Pastel: ORIG+(32+4)*3 = 140+108=248
        if (coin.symbol === 'psl' || coin.symbol === 'lsp')
            header =  new Buffer(248);
        else
            header =  new Buffer(140);
    ...
    this.getJobParams = function(){
        if (!this.jobParams){
            this.jobParams = [
                ...
                //workers - this should be in the block header, but none of the existing miners will work!!!
