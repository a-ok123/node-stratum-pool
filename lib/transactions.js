...
exports.createGeneration = (rpcData, blockReward, feeReward, recipients, poolAddress, poolHex, coin,
                            masternodeReward, masternodePayee, masternodePayment,
                            governanceReward, governancePayee,
                            zelnodeBasicAddress, zelnodeBasicAmount, zelnodeSuperAddress, zelnodeSuperAmount, zelnodeBamfAddress, zelnodeBamfAmount) => {
    ...
    if (coin.symbol === 'psl' || coin.symbol === 'lsp') {
        ...
        console.log('blockReward.total (' + blockReward.total + ')');
        console.log('blockReward.total rounded (' + Math.round(blockReward.total) + ')');

        let minerReward = Math.round(blockReward.total * (1 - (feePercent / 100)))
        console.log('blockReward.total rounded - pool fee (' + minerReward + ')');

        if (feeReward > 0) {
            if (masternodeReward > 0) {
                masternodeReward = Math.floor((blockReward.total+feeReward)/5)
            }
            if (governanceReward > 0) {
                governanceReward = Math.floor((blockReward.total+feeReward)/20)
            }
        }
        minerReward = minerReward + feeReward
        console.log('minerReward rounded - pool fee + tnx fee (' + minerReward + ')');

        minerReward = minerReward - masternodeReward - governanceReward
        console.log('minerReward rounded - pool fee + tnx fee - mnReward - govReward (' + minerReward + ')');
        console.log('masternodeReward (' + masternodeReward + ')');
        console.log('governanceReward (' + governanceReward + ')');
        ...
        if (governanceReward > 0) {
            var governanceAddrHash = bitcoin.address.fromBase58Check(governancePayee).hash;
            txb.addOutput(
                scriptCompile(governanceAddrHash),
                governanceReward
            );
        }
    }
